...
** TODO bughunt
Resume at sample_tables_defs.py, bottom.
  That data is more real in two senses.
  Try data that offers just one of them.
Also try working into the two called functions --
  it seems likely that the problem is in

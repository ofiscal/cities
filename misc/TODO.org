...
*** TODO refactor aggregation interface
Rather than distinguishing these:
  exactly_n_subcodes()
  first_n_proper_subcodes()
instead use a single function that returns all of:
  the subcode-list
  a bool for whether those subcodes are a proper subset of the input
If the list is empty, then no match was found,
  so ignore the bool.
Do likewise for the ingreso codes.
*** codes to aggregate
**** Ingresos
TI.A.1
TI.A.2

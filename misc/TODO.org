...
** sum all observations items within a (year,muni,item) cell
...
**** one strategy: compute a sum
For the sake of simplicity in the codebase,
just sum the 2-level funcionamiento items,
rather than worry about parsing their total from the raw data.
(That's because the code "VAL" used to denote the total of all ingresos
is also used to mean other things.
Hopefully, it can be differentiated with the
"INGRESOS TOTALES" value of the "concepto" (not codigo concepto) column,
but then I'd have to read that whole column into the db,
and it has super-long strings, which would slow the process.
Also I'd have to test that I'm reading it correctly, which takes time.)
**** another: use the "Concepto" (not codigo) column
Create a dummy, as early as possible,
for whether it contains (case-insensitive) the string "total".
List all Concepto values which include it,
to make sure I'm not counting anything I shouldn't.
Then use that column, rather than item code, for functionamiento spending.

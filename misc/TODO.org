...
*** DONE replace `regex_for_at_least_n_codes` with
...
*** DONE Replace `ingreso_regex` with similar
*** DONE in the last part of conceptos_1.py
Build those three columns.
Delete rows for which neither "top" nor "child" are true.
*** DONE in conceptos_2_agg.py
Aggregate on year, muni, dept, concepto-code and concepto-code-top.
*** TODO Verify that top + child = 1 (after deleting rows).

*** TODO celete "codigo-child"
Itshould be redundant to "top",
  and putting it through .agg(sum) downstream is confusing.
*** maybe rename codigo* to subcode*

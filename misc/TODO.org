...
* DONE ? determine the range of legend sizes
** why and how
...
wc */*/gastos* | sort -n -r | head -n 1
wc */*/gastos* | sort -n -r | tail -n 1
** results
...
* TODO are non-regalias ingresos disappearing for munis? (not depts)
For the full sample they're not gone.
For small samples, they're gone after budget_5 and thereafter,

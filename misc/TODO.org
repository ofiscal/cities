...
** TODO BUG : most paired gastos in Otros
Villapinzon, Cundinamarca, 1/100 sample
It's good at stage 7 (grouped), bad at stage 8 (disagg by place)
Stage 8 is good.
gastos-pct-ungrouped.csv is good, but -compare is not.
The bug is somewhere after static_muni_ungrouped() and not after
static_muni_pair().

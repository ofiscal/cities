* docker: conda update --all -y
** bump pandas to 0.25.0, other stuff
* TODO explore the data
** TODO check, then delete duplicative columns
*** TODO remaining: see "TODO" in Code/explore_dup_columns.py
*** the algorithm
For each of the three series varieties,
find the intersection of its columns with the set of duplicative columns.
Create a single dataset that includes only those columns,
from all years and all 3 series.
Aggregate on each pair of column names, discarding other columns.
Create a unit index.
Aggregate on each of the two columns, discarding the other.
Keep only rows for which the unit index is greater than unity.
If there are none, perfect.
If there are some, identify the discrepancies.
*** the columns

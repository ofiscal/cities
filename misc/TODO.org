...
* TODO build data
** TODO build a (muni code) -> (muni,dept) key
Then delete the latter two from the data before aggregating.
** TODO build a map from "codigo concepto" to "concepto"
Aside from "VAL", each code maps to only one concept,
and "VAL" can be discarded.

...
* TODO : next
** TODO fix dimensions test at end of stage 6p7
Keep every muni for with non-dept data.
Group by (dept,year,item categ).
Count the number of rows in that -- call it N.
Number of rows in pct file should be N greater than the non-pct file.
** TODO integ tests
*** for stages

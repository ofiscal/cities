...
***** TODO use pandas.Series.str.extract
Hopefully it's faster than using a lambda with a regex match.
Otherwise each build will take more than an hour, given that 3000 rows takes 5 seconds.
***** idea

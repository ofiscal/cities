...
** TODO check that "item recaudo", "item total", "item oblig" are not hard-coded
** DONE add deuda as a gasto: it's a new data set
https://sisfut.dnp.gov.co/app/reportes/categoria
"FUT_SERVICIO_DEUDA"
Take the top category, "T".
** TODO Juan's check is giving a different "other" result for Antioquia
** convert things to .csv from command line, not Open Office
...
regalias, dept
regalias, muni
** send a few sample data points to Juan for confirmation from each data set
One with decimal, one without, if the row has both kinds.
*** check regalias magnitude
 Bogota should be ~3.4e12
 Tubara should be ~1.5e10

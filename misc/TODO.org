* notes on new regalias spending data
some is dept-level, includes missing value for muni
...
AssertionError
** what to do about 3-4% of rows missing muni code?
That must be dept-level data. But do we use it?

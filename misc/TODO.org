...
But fortunately the regalias data lumps 2013 with 2014, and 2015 with 2016.
...
* TODO fix
** TODO muni codes not present
*** TODO use less state
Define functions to extract data,
which output dictionaries like "dfs".
Then I won't have to compare the test code to the original.
*** the data problem
slightly missing in raw data
entirely missing in output/conceptos_1
*** code that's good
conceptos_1 works to here:
   ###### Build aggregated concepto-code columns.
and sometimes past, but it changes.
...
** sum only first-generation descendents of aggregate conceptos

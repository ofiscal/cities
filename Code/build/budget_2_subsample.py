...
# we shrink the sample by roughly a factor of 10
...
for subsample in [1000, 100, 10]: # smallest first, to catch errors faster
  ...
  if True: # build `places` : Set[ (dept code, muni code) ]
    munis_df_subsample = defs.subsample ( subsample = subsample,
                                          df = munis_df )
    depts_df_subsample = defs.subsample (
      subsample = min(subsample,30), # to ensure there is at least one
      df = depts_df )
    places = pd.concat ( [ munis_df_subsample,
                           depts_df_subsample ] )
  for s in s2.series:
    df = dfs[s.name]
    df = df.merge ( # restrict `df` to the places in `places`
      places,

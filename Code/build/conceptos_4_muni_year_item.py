...
for s in sm.series:
  dfs[s] = pd.read_csv(
    "output/conceptos_3_muni_year_categ_top/recip-" + str(c.subsample) +
    "/" + s + ".csv" )
...
for (s, item_variety) in [
  ...
  df_myi_summaries[s] . to_csv(
    "output/conceptos_4_muni_year_categ/recip-" + str(c.subsample)
    + "/" + s + ".csv",
    index = False )
  df_summaries[s] = ( u.myDescribe( df_myi_summaries[s] ) .
                      transpose() )
  df_summaries[s] . to_csv(
    "output/conceptos_4_muni_year_categ_summary/recip-" + str(c.subsample)
    + "/" + s + ".csv" )
